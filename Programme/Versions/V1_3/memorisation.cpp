/*
    Description :
    -------------
    Ce fichier gère la sauvegarde et le chargement des paramètres
    d'arrosage dans le NVS (Non-Volatile Storage) de l'ESP32,
    permettant de conserver les paramètres après redémarrage.

    Fonctions principales :
    -----------------------
    - `initNVS()`            : Initialise le NVS pour stocker les paramètres sous le namespace "arrosage".
    - `sauvegarderParametres()` : Enregistre les paramètres d'arrosage de chaque ligne (heure, minute, durée, état, fréquence) dans le NVS.
    - `chargerParametres()`   : Charge les paramètres sauvegardés du NVS vers les variables en mémoire.
    - `closeNVS()`           : Termine l'accès au NVS pour libérer les ressources.
